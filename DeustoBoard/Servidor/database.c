...
Partida* getListaPartidaJuego(char* juego){
    sqlite3 *db;
	sqlite3_stmt *stmt;
    char sql2[] = "select * from PARTIDA where JUEGO = ?;COMMIT";
    char sql3[] = "select count(*) from PARTIDA where JUEGO = ?;COMMIT";
    char sql4[] = "select * from PARTICIPA where CODPARTIDA = ?;COMMIT";
    char sql5[] = "select count(*) from PARTICIPA;COMMIT";
    ...
    sqlite3_exec(db, "PRAGMA journal_mode=WAL;", 0, 0, 0);
	sqlite3_prepare_v2(db, sql3, strlen(sql3), &stmt, NULL) ;
    sqlite3_bind_text(stmt, 1, juego, strlen(juego), SQLITE_TRANSIENT);
    sqlite3_step(stmt);
	printf("\n");
    int count = sqlite3_column_int(stmt, 0);
    Partida* PartList = (Partida*)malloc((count) * sizeof(Partida));
...
int getTamanoListaTorneo(char *juego)
{
    sqlite3 *db;
	sqlite3_stmt *stmt;
